...
void main() {
    ...
    // Glowing purple tint (approx #915EFF) with intensity boost
    vec3 purpleTint = vec3(0.57, 0.37, 1.0) * 1.5;
    
    outColor = vec4(gray * purpleTint, texColor.a);
    outColor.a *= vAlpha;
}
`;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="infinite-menu-container" style={{ position: 'relative', width: '100%', height: '100%' }}>
      {/* Subtle background glow to match Certificates section */}
      <div style={{
          position: 'absolute',
          top: '50%',
          left: '50%',
      ...
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} style={{ position: 'relative', zIndex: 1 }} />

      {activeItem && (
        <div className={`active-item-info ${isMoving ? 'inactive' : 'active'}`}>
          <TrueFocus 
            sentence={`${activeItem.title}^${activeItem.description}`} 
            separator="^"
            manualMode={false} 
            blurAmount={4} 
            borderColor="#00d4ff" 
